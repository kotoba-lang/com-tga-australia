(ns tga-australia.main
  "Kotodama WASM entrypoint for the tga_australia clean-room actor (L4) — Clojure port.
  ...
  facts (`tga_australia.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "tga_australia")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"tga_australia.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "tga_australia-compat" :tier tier :entities entities} 200])
...
(defn start! [] :tga_australia-compat/ready)
